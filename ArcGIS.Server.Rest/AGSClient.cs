...
namespace ArcGIS.Server.Rest
{
    public class AGSClient
    {
        ...
        /// </summary>
        /// <param name="serverUrl">Administrative endpoint of the form http(s)://server/arcgis/admin</param>
        ...
        public async Task<JObject> DeleteUsageReport(string usageReport)
        {
            Uri endpoint = new Uri(ServerUrl, string.Format("usagereports/{0}/delete", usageReport));
        ...
        public async Task<ReportResponse> QueryUsageReport(string usageReport)
        {

            Uri endpoint = new Uri(ServerUrl, string.Format("usagereports/{0}/data", usageReport));

            Dictionary<string, string> d = new Dictionary<string, string>();
            d.Add("filter", "{\"machines\": \"*\"}");
            var content = new FormUrlEncodedContent(d);
            return await PostAsync<ReportResponse>(endpoint, content, true);
        }
        ...
        /// <summary>
        /// check if the service is in realTimeState Stopped
        /// </summary>
        /// <param name="service">name of service</param>
        /// <returns>if service is realTimeState Stopped return true</returns>
        public async Task<bool> IsStopped(string service)
        {
            Status status = await StatusService(service);

            return status.realTimeState == ServiceStatus.Stopped;
        }
        ...
        public async Task<Status> StatusService(string service)
        {
            Uri endPoint = new Uri(ServerUrl, string.Format("services/{0}/status", service));
            Status status = await GetStringAsync<Status>(endPoint);
        ...
        /// </summary>
        /// <param name="service">name of service</param>
        /// <returns>resutl of operation after start service</returns>
        public async Task<StatusService> Start(string service)
        {

            Uri endPoint = new Uri(ServerUrl, string.Format("services/{0}/start", service));

            return await PostAsync<StatusService>(endPoint, null, true);
        }
        ...
        internal async Task<Uri> FixupUri(Uri uri, IEnumerable<KeyValuePair<string, string>> param = null, bool addToken = true)
        {
            ...
            var queryString = HttpUtility.ParseQueryString(uri.Query.ToString());
